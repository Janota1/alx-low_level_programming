...
/**
 * _strncmp - compars different strings
 * @s1:  string 1
 * @s2:  string 2
 * @b: the max number of bytes to compare
 *
 * Return: 0 if the first b bytes of s1 and s2 are equal, otherwise non-zero
 */
int _strncmp(const char *s1, const char *s2, size_t b)
{
	for ( ; b && *s1 && *s2; --b, ++s1, ++s2)
	{
		if (*s1 != *s2)
